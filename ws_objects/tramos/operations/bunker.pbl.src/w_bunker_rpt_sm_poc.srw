$PBExportHeader$w_bunker_rpt_sm_poc.srw
$PBExportComments$Window for bunker report sharemembers. Let the user enter informations, retrieves the report and print it.
forward
global type w_bunker_rpt_sm_poc from w_bunker_rpt_base
end type
type dw_sharemembers from mt_u_datawindow within w_bunker_rpt_sm_poc
end type
end forward

global type w_bunker_rpt_sm_poc from w_bunker_rpt_base
string title = "Bunker Report Sharemember (PoC)"
dw_sharemembers dw_sharemembers
end type
global w_bunker_rpt_sm_poc w_bunker_rpt_sm_poc
	...
	IF ll_row_count_sharemembers > 0 THEN
		IF lds_sharemembers.GetItemNumber(1, "ntc_tc_period_rate") > 0 THEN
			FOR ll_row_sharemembers = 1 TO ll_row_count_sharemembers
				...
				dw_sharemembers.SetItem(ll_new_row,"bunkerstockused",dw_report.GetItemNumber(ll_row, "bunkerstockused"))
				dw_sharemembers.SetItem(ll_new_row,"isvalidbunkerstock",dw_report.GetItemNumber(ll_row, "isvalidbunkerstock"))
				dw_sharemembers.SetItem(ll_new_row,"stock_date",dw_report.GetItemDateTime(ll_row, "stock_date"))
...
public subroutine documentation ();/********************************************************************
   ObjectName: w_bunker_rpt_sm_poc
	
	<OBJECT>
		Decendent object of w_bunker_base a member of report group for Bunker Stock.  
	</OBJECT>
  	<DESC>
		Business refer to this as the 'PoC' Sharemember version of the Bunker Stock report.  
		This item uses bunker data held within the vessel table to assist calculations
	</DESC>
  	<USAGE>
		Pulled from menu item "Reports>Bunker Report>Bunker Report Sharemember (PoC)"
	</USAGE>
   <ALSO>
...
on w_bunker_rpt_sm_poc.destroy
call super::destroy
destroy(this.dw_sharemembers)
end on

type st_hidemenubar from w_bunker_rpt_base`st_hidemenubar within w_bunker_rpt_sm_poc
end type

type cbx_excludetcout from w_bunker_rpt_base`cbx_excludetcout within w_bunker_rpt_sm_poc
boolean visible = false
integer taborder = 0
end type

type cbx_exclude from w_bunker_rpt_base`cbx_exclude within w_bunker_rpt_sm_poc
end type

type st_status from w_bunker_rpt_base`st_status within w_bunker_rpt_sm_poc
end type

type hpb_1 from w_bunker_rpt_base`hpb_1 within w_bunker_rpt_sm_poc
end type

type cb_saveas from w_bunker_rpt_base`cb_saveas within w_bunker_rpt_sm_poc
end type
...
type st_1 from w_bunker_rpt_base`st_1 within w_bunker_rpt_sm_poc
end type

type dw_date from w_bunker_rpt_base`dw_date within w_bunker_rpt_sm_poc
end type

type cb_print from w_bunker_rpt_base`cb_print within w_bunker_rpt_sm_poc
end type

event cb_print::clicked;dw_sharemembers.print(true)
end event

type cb_retrieve from w_bunker_rpt_base`cb_retrieve within w_bunker_rpt_sm_poc
end type
...
type dw_finresp from w_bunker_rpt_base`dw_finresp within w_bunker_rpt_sm_poc
end type

type dw_saveas from w_bunker_rpt_base`dw_saveas within w_bunker_rpt_sm_poc
string dataobject = "d_ex_tb_bunker_rpt_sm_poc_saveas"
end type

type dw_imolist from w_bunker_rpt_base`dw_imolist within w_bunker_rpt_sm_poc
end type

type cbx_selectall_pc from w_bunker_rpt_base`cbx_selectall_pc within w_bunker_rpt_sm_poc
end type

type gb_profitcenter from w_bunker_rpt_base`gb_profitcenter within w_bunker_rpt_sm_poc
end type

type dw_pc from w_bunker_rpt_base`dw_pc within w_bunker_rpt_sm_poc
end type

type gb_criteria from w_bunker_rpt_base`gb_criteria within w_bunker_rpt_sm_poc
end type

type st_3 from w_bunker_rpt_base`st_3 within w_bunker_rpt_sm_poc
end type

type rb_finresp from w_bunker_rpt_base`rb_finresp within w_bunker_rpt_sm_poc
end type

type rb_vessel from w_bunker_rpt_base`rb_vessel within w_bunker_rpt_sm_poc
end type

type sle_vessels from w_bunker_rpt_base`sle_vessels within w_bunker_rpt_sm_poc
end type

type cb_sel_vessel from w_bunker_rpt_base`cb_sel_vessel within w_bunker_rpt_sm_poc
end type

type dw_info from w_bunker_rpt_base`dw_info within w_bunker_rpt_sm_poc
end type

type dw_report from w_bunker_rpt_base`dw_report within w_bunker_rpt_sm_poc
boolean visible = false
integer x = 201
integer y = 620
integer width = 1591
...
string dataobject = "d_ex_tb_bunker_rpt_sm_poc"
