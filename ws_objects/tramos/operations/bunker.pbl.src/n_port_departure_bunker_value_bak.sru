$PBExportHeader$n_port_departure_bunker_value_bak.sru
$PBExportComments$Calculate bunker values when vessel depart from a given portcall.
forward
global type n_port_departure_bunker_value_bak from mt_n_nonvisualobject
end type
end forward

global type n_port_departure_bunker_value_bak from mt_n_nonvisualobject
end type
global n_port_departure_bunker_value_bak n_port_departure_bunker_value_bak
...
public function decimal of_calculate (integer ai_vesselnr, string as_voyagenr, datetime adt_port_arr, decimal ad_bunker_hsfo, decimal ad_bunker_lsgo, decimal ad_bunker_hsgo, decimal ad_bunker_lsfo)
end prototypes
...
public function decimal of_calculate (integer ai_vesselnr, string as_voyagenr, datetime adt_port_arr, decimal ad_bunker_hsfo, decimal ad_bunker_lsgo, decimal ad_bunker_hsgo, decimal ad_bunker_lsfo);/********************************************************************
   of_calculate
   <DESC>This function calculates the price for bunker on board at arrival at given port 
			  The calculation is splitted into several steps
   </DESC>
...
long 			ll_liftrows, ll_liftrow, ll_bunker_counter
decimal{4}	ld_bunker_on_board, ld_price, ld_sum_amount
string			ls_bunkertypes[4] = {"HFO", "DO", "GO", "LSHFO"}  

ii_vessel   = ai_vesselnr
is_voyage = as_voyagenr
...
for ll_bunker_counter = 1 to 4
	...
	choose case is_bunkertype
		case "HFO"
			ld_bunker_on_board = ad_bunker_hsfo
		case "DO"
			ld_bunker_on_board = ad_bunker_lsgo
		case "GO"
			ld_bunker_on_board = ad_bunker_hsgo
		case "LSHFO"
			ld_bunker_on_board = ad_bunker_lsfo
	end choose
	
	of_initialize( )

	if ld_bunker_on_board = 0 then continue		
	
	ids_bunker_lifted.id_stop_load = ld_bunker_on_board
	ll_liftrows = ids_bunker_lifted.retrieve(ii_vessel, is_voyage, adt_port_arr)
	...
	if not isNull(ld_price) then 
		ld_sum_amount += ld_price
	end if
next

return ld_sum_amount
end function

on n_port_departure_bunker_value_bak.create
call super::create
end on

on n_port_departure_bunker_value_bak.destroy
call super::destroy
end on
