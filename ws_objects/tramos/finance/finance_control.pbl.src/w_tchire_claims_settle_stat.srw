$PBExportHeader$w_tchire_claims_settle_stat.srw
$PBExportComments$Shows freight receivables statistics
forward
global type w_tchire_claims_settle_stat from window
end type
type em_days from editmask within w_tchire_claims_settle_stat
end type
type st_10 from statictext within w_tchire_claims_settle_stat
end type
type rb_days_greater from radiobutton within w_tchire_claims_settle_stat
end type
type rb_days_less from radiobutton within w_tchire_claims_settle_stat
end type
type em_year from editmask within w_tchire_claims_settle_stat
end type
type st_4 from statictext within w_tchire_claims_settle_stat
end type
type rb_less from radiobutton within w_tchire_claims_settle_stat
end type
type rb_equal from radiobutton within w_tchire_claims_settle_stat
end type
type rb_greater from radiobutton within w_tchire_claims_settle_stat
end type
type cb_saveas from commandbutton within w_tchire_claims_settle_stat
end type
type cb_sel_office from commandbutton within w_tchire_claims_settle_stat
end type
type cb_sel_broker from commandbutton within w_tchire_claims_settle_stat
end type
type cb_sel_chart from commandbutton within w_tchire_claims_settle_stat
end type
type cbx_include_office from checkbox within w_tchire_claims_settle_stat
end type
type sle_offices from singlelineedit within w_tchire_claims_settle_stat
end type
type st_9 from statictext within w_tchire_claims_settle_stat
end type
type st_8 from statictext within w_tchire_claims_settle_stat
end type
type sle_brokers from singlelineedit within w_tchire_claims_settle_stat
end type
type cbx_include_broker from checkbox within w_tchire_claims_settle_stat
end type
type cb_sel_vessel from commandbutton within w_tchire_claims_settle_stat
end type
type cbx_include_chart from checkbox within w_tchire_claims_settle_stat
end type
type cbx_include_vessel from checkbox within w_tchire_claims_settle_stat
end type
type sle_charterers from singlelineedit within w_tchire_claims_settle_stat
end type
type st_7 from statictext within w_tchire_claims_settle_stat
end type
type sle_vessels from singlelineedit within w_tchire_claims_settle_stat
end type
type st_6 from statictext within w_tchire_claims_settle_stat
end type
type cb_refresh from commandbutton within w_tchire_claims_settle_stat
end type
type cb_print from commandbutton within w_tchire_claims_settle_stat
end type
type cb_close from commandbutton within w_tchire_claims_settle_stat
end type
type st_1 from statictext within w_tchire_claims_settle_stat
end type
type dw_profit_center from datawindow within w_tchire_claims_settle_stat
end type
type dw_claims_settled from datawindow within w_tchire_claims_settle_stat
end type
type gb_3 from groupbox within w_tchire_claims_settle_stat
end type
type gb_2 from groupbox within w_tchire_claims_settle_stat
end type
type ln_1 from line within w_tchire_claims_settle_stat
end type
type ln_2 from line within w_tchire_claims_settle_stat
end type
type ln_3 from line within w_tchire_claims_settle_stat
end type
type ln_4 from line within w_tchire_claims_settle_stat
end type
end forward
...
string title = "T/C Hire Settle Stat"
...
global w_tchire_claims_settle_stat w_tchire_claims_settle_stat
...
if (len(em_year.text) > 0) then
	...
	if (len(ls_filter)=0) then
		ls_filter += "compute_voyage_sort" + ls_operator + " '" + string(em_year.text) +"'"
	else
		ls_filter += " and "
		ls_filter += " compute_voyage_sort " + ls_operator +  " '" + string(em_year.text) +"'"
	end if
...
if (len(sle_vessels.text) > 0) then
	if cbx_include_vessel.checked then
		if (len(ls_filter)=0) then
			ls_filter += " vessels_vessel_nr in ( " + string(is_vessel_filter) + " )"
		else 
			ls_filter += " and "
			ls_filter += " vessels_vessel_nr in ( " + string(is_vessel_filter) + " )"
		end if
		...
		if (len(ls_filter)=0) then
			ls_filter += " vessels_vessel_nr not in ( " + string(is_vessel_filter) + " )"
		else 
			ls_filter += " and "
			ls_filter += " vessels_vessel_nr not in ( " + string(is_vessel_filter) + " )"
		end if
...
if (len(sle_charterers.text) > 0) then
	if cbx_include_chart.checked then
		if (len(ls_filter)=0) then
			ls_filter += " chart_chart_nr in ( " + string(sle_charterers.text) + " )"
		else 
			ls_filter += " and "
			ls_filter += " chart_chart_nr in ( " + string(sle_charterers.text) + " )"
		end if
		...
		if (len(ls_filter)=0) then
			ls_filter += " chart_chart_nr not in ( " + string(sle_charterers.text) + " )"
		else 
			ls_filter += " and "
			ls_filter += " chart_chart_nr not in ( " + string(sle_charterers.text) + " )"
		end if
...
on w_tchire_claims_settle_stat.create
...
on w_tchire_claims_settle_stat.destroy
...
event clicked;dw_claims_settled.SaveAs()
end event
...
integer taborder = 60
string title = "none"
string dataobject = "d_tchire_claims_settled"
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type
