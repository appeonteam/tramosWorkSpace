$PBExportHeader$u_fin_tc_voyage.sru
forward
global type u_fin_tc_voyage from userobject
end type
type cb_generateest from mt_u_commandbutton within u_fin_tc_voyage
end type
type cb_jump_disbursement from commandbutton within u_fin_tc_voyage
end type
type st_1 from statictext within u_fin_tc_voyage
end type
type tab_finance from tab within u_fin_tc_voyage
end type
...
type tabpage_tccomm from userobject within tab_finance
end type
type dw_tccomm from datawindow within tabpage_tccomm
end type
type tabpage_tccomm from userobject within tab_finance
dw_tccomm dw_tccomm
end type
...
tabpage_tccomm tabpage_tccomm
tabpage_tcin tabpage_tcin
tabpage_voyage_notes tabpage_voyage_notes
end type
type dw_charterer from datawindow within u_fin_tc_voyage
end type
type gb_1 from groupbox within u_fin_tc_voyage
end type
type gb_2 from groupbox within u_fin_tc_voyage
end type
end forward
...
global u_fin_tc_voyage u_fin_tc_voyage
...
forward prototypes
public subroutine of_setvesselvoyage (integer ai_vesselnr, string as_voyagenr)
...
public subroutine of_setvesselvoyage (integer ai_vesselnr, string as_voyagenr);ii_vessel_nr = ai_vesselnr
...
ll_rows = dw_charterer.retrieve(ii_vessel_nr, is_voyage_nr)
//if ll_rows > 0 then idt_cpdate = dw_charterer.getItemDatetime(1, "tchires_tchire_cp_date")
commit;
...
lstr_app.s_estimatetype = "tcout"
lstr_app.i_clientvesselnr = ai_vesselnr
lstr_app.s_clientvoyagenr = as_voyagenr
lstr_app.s_infomessage = "Successfully generated estimated tc_out transaction for selected vessel & voyage to send to AX."
...
if lstr_app.s_returnmessage<>"" then
	...
	messagebox("Info", lstr_app.s_infomessage)
end if
destroy lnv_est
...
private subroutine documentation ();/********************************************************************
   ...
   <OBJECT>
		...
		panel. This object is used for TC out voyages and also not allocated voayges.
	</OBJECT>
...
tab_finance.tabpage_tccomm.dw_tccomm.setTransObject(SQLCA)
...
event clicked;call super::clicked;if dw_charterer.rowcount( ) > 0 then
	if dw_charterer.getitemnumber(1,"ntc_tc_contract_tc_hire_in") = 0 then
		_generateestimates(ii_vessel_nr,left(is_voyage_nr,5))
	else
		messagebox("Info","This is not a TC-Out voyage.  Can not generate estimate")
	end if
else
	messagebox("Info","No charterer exists.  Can not generate estimate")
end if
...
this.tabpage_tccomm=create tabpage_tccomm
...
event ue_retrieve;long ll_contract_id


IF dw_charterer.RowCount() > 0 then
	ll_contract_id = dw_charterer.getItemNumber(1, "ntc_tc_contract_contract_id")
end if

this.enabled = dw_tccomm.retrieve(ll_contract_id) > 0

COMMIT;
end event

on tabpage_tccomm.create
this.dw_tccomm=create dw_tccomm
this.Control[]={this.dw_tccomm}
end on

on tabpage_tccomm.destroy
destroy(this.dw_tccomm)
end on
...
integer taborder = 20
string title = "none"
string dataobject = "dw_fin_tc_commission"
boolean vscrollbar = true
boolean livescroll = true
borderstyle borderstyle = stylelowered!
end type

event doubleclicked;long ll_broker_nr
long ll_paymentid
u_jump_commission luo_jump_commission

if row < 1 then return

ll_broker_nr = this.getItemNumber(row, "broker_nr")
ll_paymentid = this.getItemNumber(row, "payment_id")

luo_jump_commission = CREATE u_jump_commission
luo_jump_commission.of_open_tc_commission(ll_broker_nr, ll_paymentid)
