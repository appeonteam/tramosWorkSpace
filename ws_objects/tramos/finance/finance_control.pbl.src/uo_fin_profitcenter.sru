$PBExportHeader$uo_fin_profitcenter.sru
forward
global type uo_fin_profitcenter from userobject
end type
type rb_header2 from radiobutton within uo_fin_profitcenter
end type
type rb_header1 from radiobutton within uo_fin_profitcenter
end type
type dw_1 from uo_datawindow within uo_fin_profitcenter
end type
type sle_find from singlelineedit within uo_fin_profitcenter
end type
type st_1 from statictext within uo_fin_profitcenter
end type
type gb_1 from groupbox within uo_fin_profitcenter
end type
end forward
...
global uo_fin_profitcenter uo_fin_profitcenter
...
IF ii_current_column = 1 THEN
	dw_1.SetSort("pc_nr"+" A")	
ELSE
	dw_1.SetSort("pc_name"+" A")
END IF
dw_1.Sort()
...
//istr_parametre.list_window = "dw_profit_center_list"
...
//istr_parametre.column_name[0] = "Number"
//istr_parametre.column_name[1] = "Name"
...
//rb_header1.text = istr_parametre.column_name[0]
//rb_header2.text = istr_parametre.column_name[1]
...
on uo_fin_profitcenter.destroy
...
integer height = 564
integer taborder = 20
string dataobject = "d_fin_profitcenter"
...
IF rb_header1.checked = TRUE THEN
	sle_find.text = string(dw_1.GetItemNumber(il_selectedrow, "pc_nr"))
ELSE
	sle_find.text = dw_1.GetItemString(il_selectedrow, "pc_name")
END IF

is_filter = sle_find.text
	
ll_array[1] = dw_1.getitemnumber(row, "pc_nr")
...
If li_movement <> 0 Then
	...
	Else
		sle_find.text = dw_1.GetItemString(ll_row, "pc_name");
	END IF
...
IF rb_header1.checked = true THEN
		ll_found_row = dw_1.Find("pc_nr"+">="+is_filter,1, 99999)
ELSE
		ll_found_row = dw_1.Find("pc_name"+">='"+Lower(is_filter) + "'",1, 99999)
END IF
