...
private function integer wf_load_systemvars (ref string as_servername, ref string as_database, ref string as_dbms, ref string as_dbparm)
end prototypes
...
public subroutine documentation ();/********************************************************************
	...
	<HISTORY>
		...
		05/09/16		CR3754		AGL027		Single Sign On modifications
		18/11/16		CR3754		AGL027		SSO - cover instances where 1) user may not be inside USERS table and 2) user is not member of tramos group in AD 
	</HISTORY>
********************************************************************/
...
as_dbms = "ASE ADAPTIVE SERVER ENTERPRISE"
as_dbparm = "OJSyntax='PB',release='15',UTF8=1"
if ib_use_sso then
	as_dbparm += ",Sec_Cred_Timeout=100,Sec_Network_Auth=1,Sec_Server_Principal='" + uo_global.is_sec_server_principal + "',Host='TramosApp' Appname='TramosApp'"	
end if
...
25/08-96		3.0			minbox			Changed to CTL3DV2 look
22/04-13		3158			WWA048	Update the UI of login window.
16/08/16		CR3754		AGL027	Single Sign On modifications
*********************************************/
...
if not keydown(keyShift!) then
	if uo_global.is_sec_server_principal<>"" then
		ib_use_sso = true
		this.visible=false
...
event clicked;/********************************************************************
   ...
   <USAGE> </USAGE>
	<HISTORY> 
		...
		16/06/15		CR3694		XSZ004		Change password every 90 days instead of 30 days.
		16/08/16		CR3754		AGL027		Single Sign On integration
	</HISTORY>   
********************************************************************/
...
if trim(ls_userid) = "" then
	...
	return
elseif trim(ls_psw) = "" and not ib_use_sso then
...
uo_global.is_logid = ls_userid
uo_global.is_password = ls_psw

if wf_load_systemvars(uo_global.is_servername, uo_global.is_database, uo_global.is_dbms, uo_global.is_dbparm)=-1 then return
...
SQLCA.Logid      = uo_global.is_logid
SQLCA.LogPass    = uo_global.is_password
SQLCA.Dbparm     = uo_global.is_dbparm

if not ib_use_sso then
...
if sqlca.sqlcode = 100 then // null value returned
	messagebox("Access Denied","The user id " + uo_global.is_userid + " does not exist in TRAMOS. Please contact TRAMOS support to request access.", StopSign!)
	ii_attempts ++
	dw_login.setcolumn("user_id")
	//if uo_global.is_sec_server_principal then
	DISCONNECT USING SQLCA;
...
if lb_deleted then
	messagebox("Access Denied", "Your user profile has been deactivated. Please contact Service Desk to re-instate your profile.", StopSign!)
	ii_attempts ++
	dw_login.setcolumn("user_id")
	DISCONNECT USING SQLCA;
	if ib_use_sso then
...
if daysafter(date(ldt_pw_changed), today()) > 90 & 
	and uo_global.is_userid <> "sa" and ldt_dummy <> ldt_pw_changed and not ib_use_sso then
