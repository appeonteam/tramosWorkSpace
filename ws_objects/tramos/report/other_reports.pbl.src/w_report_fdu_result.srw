$PBExportHeader$w_report_fdu_result.srw
$PBExportComments$FDU Report - (menu: report.fdu report)- de data der sendes til norgas
forward
global type w_report_fdu_result from window
end type
type cb_close from commandbutton within w_report_fdu_result
end type
type cb_saveas from commandbutton within w_report_fdu_result
end type
type cb_print from commandbutton within w_report_fdu_result
end type
type dw_tce_report from datawindow within w_report_fdu_result
end type
type dw_vas_report from datawindow within w_report_fdu_result
end type
type cb_create from commandbutton within w_report_fdu_result
end type
type dw_select from datawindow within w_report_fdu_result
end type
type st_2 from statictext within w_report_fdu_result
end type
type st_1 from statictext within w_report_fdu_result
end type
type dw_enddate from datawindow within w_report_fdu_result
end type
type dw_startdate from datawindow within w_report_fdu_result
end type
type rb_pool from radiobutton within w_report_fdu_result
end type
type rb_profitcenter from radiobutton within w_report_fdu_result
end type
type gb_1 from groupbox within w_report_fdu_result
end type
end forward
...
global w_report_fdu_result w_report_fdu_result
...
IF ll_found < ll_rowcount AND ll_found > 0 THEN
	...
	IF lds_extra_voyage_vessel.getItemNumber(ll_found + 1, "voyages_voyage_finished") = 0 AND IsNull(lds_extra_voyage_vessel.getItemNumber(ll_found + 1, "voyages_cal_calc_id")) AND lds_extra_voyage_vessel.getItemNumber(ll_found + 1, "voyages_voyage_type")=1 THEN
	   ...
	   IF adt_voyage_start > adt_end THEN
			ld_sum_revenu = 0
		ELSEIF ldt_voyage_end < adt_start THEN
			ld_sum_revenu = 0
		ELSEIF adt_voyage_start <= adt_start and ldt_voyage_end >= adt_end THEN
			ld_sum_revenu = (f_timedifference(adt_start,adt_end)/1440) - ld_sum_idle - ld_sum_offhire
		ELSEIF adt_voyage_start >= adt_start and ldt_voyage_end <= adt_end THEN 	
			ld_sum_revenu = (f_timedifference(adt_voyage_start,ldt_voyage_end)/1440) - ld_sum_idle - ld_sum_offhire
		ELSEIF adt_voyage_start < adt_start and ldt_voyage_end > adt_start THEN 		
			ld_sum_revenu = (f_timedifference(adt_start,ldt_voyage_end)/1440) - ld_sum_idle - ld_sum_offhire
		ELSEIF adt_voyage_start < adt_end and ldt_voyage_end > adt_end THEN 		
	...
	adw_report.setItem(ll_tce_row, "revenue_days", ld_sum_revenu + ld_sum_idle + ld_sum_offhire)
...
//		adw_report.setItem(ll_tce_row, "revenue_days", 0)
		...
		//
		adw_report.setItem(ll_tce_row, "revenue_days",(ld_report_days - ld_days) - ld_remaining_report_days)			
...
on w_report_fdu_result.destroy
...
for ll_counter = 1 to ll_voyages
	...
	IF lds_working.getItemString(ll_counter, "voyage_nr") = "EXTRA" THEN  
			...
			dw_tce_report.setItem(ll_tce_row, "revenue_days", ld_idle_days + ld_offhire_days)			
