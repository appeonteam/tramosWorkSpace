$PBExportHeader$w_backup_report_fdu_result.srw
$PBExportComments$BACKUP 29/09/04 - FDU Report - (menu: report.fdu report)- de data der sendes til norgas
forward
global type w_backup_report_fdu_result from window
end type
type cb_close from commandbutton within w_backup_report_fdu_result
end type
type cb_saveas from commandbutton within w_backup_report_fdu_result
end type
type cb_print from commandbutton within w_backup_report_fdu_result
end type
type dw_tce_report from datawindow within w_backup_report_fdu_result
end type
type dw_vas_report from datawindow within w_backup_report_fdu_result
end type
type cb_create from commandbutton within w_backup_report_fdu_result
end type
type dw_select from datawindow within w_backup_report_fdu_result
end type
type st_2 from statictext within w_backup_report_fdu_result
end type
type st_1 from statictext within w_backup_report_fdu_result
end type
type dw_enddate from datawindow within w_backup_report_fdu_result
end type
type dw_startdate from datawindow within w_backup_report_fdu_result
end type
type rb_pool from radiobutton within w_backup_report_fdu_result
end type
type rb_profitcenter from radiobutton within w_backup_report_fdu_result
end type
type gb_1 from groupbox within w_backup_report_fdu_result
end type
end forward
...
global w_backup_report_fdu_result w_backup_report_fdu_result
...
public subroutine uf_insert_extra_days (integer ai_vessel_nr, string as_voyage_nr, ref datawindow adw_report, datetime adt_start, datetime adt_end, datetime adt_voyage_start, datetime adt_voyage_end, string as_vessel_name, boolean ab_pool)
end prototypes
...
public subroutine uf_insert_extra_days (integer ai_vessel_nr, string as_voyage_nr, ref datawindow adw_report, datetime adt_start, datetime adt_end, datetime adt_voyage_start, datetime adt_voyage_end, string as_vessel_name, boolean ab_pool);long ll_tce_row, ll_rowcount, ll_found, ll_rows
...
IF ll_found < ll_rowcount AND ll_found > 0 THEN
	...
	adw_report.setItem(ll_tce_row, "corrected_revenue_days", ld_sum_revenu)			
...
IF ll_rows >= 0 then
	IF ll_rows = 0 THEN
		ld_days = 0
	ELSE	
		IF adw_report.GetItemNumber(ll_rows,"vessel_nr") <> ai_vessel_nr THEN
			ld_days = 0
		ELSE
   ...
   IF	ab_pool = true THEN
	...
	IF ld_remaining_report_days <= ld_report_days - ld_days THEN
		...
		adw_report.setItem(ll_tce_row, "corrected_revenue_days", 0)	
...
on w_backup_report_fdu_result.create
...
on w_backup_report_fdu_result.destroy
...
integer taborder = 90
string title = "none"
string dataobject = "d_fdu_report"
...
for ll_counter = 1 to ll_voyages
	...
	IF li_vessel_nr_tmp <> li_vessel_nr_tmp2 AND li_vessel_nr_tmp2 <> 0 THEN
		uf_insert_extra_days(li_vessel_nr_tmp2,ls_voyage_nr_tmp2,dw_tce_report,ldt_start,ldt_end,ldt_voyage_end,ldt_end,ls_vessel_name_tmp,lb_pool)
	END IF
	...
	if isValid(w_progress) then
		w_progress.wf_progress(ll_counter/ll_voyages, "Calculating vessel #"+string(lstr_vessel_voyage_list[1].vessel_nr,"000")+" voyage #"+lstr_vessel_voyage_list[1].voyage_nr+" ...")
	end if
	dw_vas_report.reset()
	...
	IF lds_working.getItemString(ll_counter, "voyage_nr") = "EXTRA" THEN  
			...
			dw_tce_report.setItem(ll_tce_row, "corrected_revenue_days", 0)			
...
//			ld_revenue_days = dw_vas_report.getItemNumber(1, "est_act_days_total") - ld_idle_days - ld_offhire_days
			ld_revenue_days = dw_vas_report.getItemNumber(1, "est_act_days_total") 			
			...
			dw_tce_report.setItem(ll_tce_row, "idle_days", ld_idle_days)			
			dw_tce_report.setItem(ll_tce_row, "offhire_days", ld_offhire_days)			
			dw_tce_report.setItem(ll_tce_row, "corrected_revenue_days", ld_revenue_days)			
