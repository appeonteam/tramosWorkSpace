$PBExportHeader$u_transaction_opsa_hire_bareboat_coda.sru
$PBExportComments$TC Hire Out Contract with OPSA setup. Bareboat Hire is posted as extra transaction
forward
global type u_transaction_opsa_hire_bareboat_coda from u_transaction_hire
end type
end forward

global type u_transaction_opsa_hire_bareboat_coda from u_transaction_hire
end type
global u_transaction_opsa_hire_bareboat_coda u_transaction_opsa_hire_bareboat_coda
...
IF isNull(id_exrate_tc_to_DKK) OR id_exrate_tc_to_DKK = 0 THEN
	of_messagebox("Error", "Cant get Exchange Rate for TC Currency. Object: u_transaction_opsa_hire_bareboat_coda, function: of_setExrate()")
	Return -1
END IF
...
if is_tc_currency = "USD" then
	...
	IF isNull(ld_exrate_DKK_to_USD) OR ld_exrate_DKK_to_USD = 0 THEN
		of_messagebox("Error", "Cant get Exchange Rate for USD. Object: Object: u_transaction_opsa_hire_bareboat_coda, function: of_setExrate()")
		Return -1 
	END IF
...
do while month(date(ldt_calc_start)) <> month(date(f_long2datetime(f_datetime2long(adt_end)-1)))
	...
	IF of_getVoyagenr(ldt_calc_start, ls_voyage_nr) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 16 (NULL) (Element 6) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_periodiser_bpost")
		Return(-1)
	END IF
...
/* get CODA elements for Bareboat Hire */ 
if ids_coda_elements.retrieve(istr_trans_input.settle_tc_payment.getItemNumber(1, "contract_id"), 1) <> 2 then return -1
...
IF ids_apost.SetItem(1,"trans_type","TCCODARec") <> 1 THEN
	of_messagebox("Set value error", "Cant set transaction type for A-post. Object: u_transaction_opsa_hire_bareboat_coda, &
									function: of_fill_transaction")
	Return(-1)
END IF

IF ids_apost.SetItem(1,"ch_br_ag_nr", istr_trans_input.agent_no) <> 1 THEN
	of_messagebox("Set value error", "Cant set broker nr for A-post. Object: u_transaction_opsa_hire_bareboat_coda, &
									function: of_fill_transaction")
	Return(-1)
END IF

IF ids_apost.SetItem(1,"claim_pcn_nr",istr_trans_input.pcn) <> 1 THEN
	of_messagebox("Set value error", "Cant set claim nr for A-post. Object: u_transaction_opsa_hire_bareboat_coda, &
									function: of_fill_transaction")
	Return(-1)
END IF
////////////////////////////////////////////////////////////////////////////////////////
...
IF ids_apost.SetItem(1, "f06_doccode", ids_default_values.GetItemString(1, "doccode_coda_trans")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 06 (Doccode) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 9 DocDate */
IF ids_apost.SetItem(1, "f09_docdate", istr_trans_input.doc_date) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 09 (Docdate) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
IF ids_apost.SetItem(1, "f13_el3", ids_coda_elements.getItemString(ii_Arow, "opsa_el3") ) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 13 (Own) (Element 3) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 14  Nominal Account  */
IF ids_apost.SetItem(1, "f14_el4",  ids_coda_elements.getItemString(ii_Arow, "opsa_el4") ) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 14 (Foreign) (Element 4) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
ELSE
	of_messagebox("Retrieval error", "Alphanumeric Reference Number for vessel not found. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
if isNull( ids_coda_elements.getItemString(ii_Arow, "opsa_el5")) then
	IF ids_apost.SetItem(1,"f15_el5", ids_default_values.GetItemString(1,"prefix_vessel") + ls_vessel_ref_nr) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 15 (Foreign) (Element 5) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
		Return(-1)
	END IF
else
...
if isNull( ids_coda_elements.getItemString(ii_Arow, "opsa_el7")) then
	IF ids_apost.SetItem(1, "f17_el7", "") <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 17 (Foreign) (Element 7) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
		Return(-1)
	END IF
else
...
IF ids_apost.SetItem(1, "f19_custsupp", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 19 for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 20  (always blank)  */
IF ids_apost.SetItem(1, "f20_invoicenr", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 20 (Invoice No.) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 21 Voucher no. (always blank) */
IF ids_apost.SetItem(1, "f21_vouchernr", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 21 (Voucher No.) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 22 Control no. (always blank) */
IF ids_apost.SetItem(1, "f22_controlnr", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 22 (Control No.) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 23  APM Supplier - same as nominal account (always blank)   */
IF ids_apost.SetItem(1, "f23_paytype_or_sup", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 23 (APM Supplier) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 28  Currency Code  */
IF ids_apost.SetItem(1, "f28_curdoc", is_tc_currency) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 28 (CurDoc) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 41  Linedescr  */
/* This text must never be changed as it is used to identify the transaction in the log */
IF ids_apost.SetItem(1, "f41_linedesr", "TC OPSA BB (Hire)") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 41 (Linedescr) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
IF ids_apost.SetItem(1, "payment_id", istr_trans_input.settle_tc_payment.getItemNumber(1, "payment_id")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field Payment_ID for A-post. Object:  u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
IF ids_apost.SetItem(1, "f11_el1", string(idt_payment_start, "YYYYMM")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 11 (Element 1) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
if isNull(ids_coda_elements.getItemString(ii_Arow, "opsa_el6")) then
	IF of_getVoyagenr(idt_payment_start, ls_voyage_nr) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 16 (NULL) (Element 6) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
		Return(-1)
	END IF
	IF ids_apost.SetItem(1, "f16_el6", ids_default_values.GetItemString(1, "prefix_voyage") + ls_voyage_nr) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 16 (Element 6) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
		Return(-1)
	END IF
else
...
if ll_rows > 0 then
	for ll_row = 1 to ll_rows
		IF ids_bpost.SetItem(ll_row, "f08_doclinenum_b", ll_row +1) <> 1 THEN
			of_messagebox("Set value error", "Cant set Field 08 (Doclinenumber) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_bpost")
			Return(-1)
		end if
...
IF ld_sum_amount_local > 0 THEN
	IF ids_apost.SetItem(1, "f29_debitcredit", ids_default_values.GetItemNumber(1, "debitcredit_debit")) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 29 (DebitCredit) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
		Return(-1)
	END IF
ELSE
...
IF ids_apost.SetItem(1, "f30_valuedoc", abs(ld_sum_amount_local)) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 30 (Valuedoc) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 31 (Valuedoc_dp) */
IF ids_apost.SetItem(1, "f31_valuedoc_dp", 2) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 31 (Valuedoc_dp) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 32 (Valuehome) */
IF ids_apost.SetItem(1, "f32_valuehome", abs(ld_sum_amount_DKK)) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 32 (Valuehome) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 33 (Valuehome_dp) */
IF ids_apost.SetItem(1, "f33_valuehome_dp", 2) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 33 (Valuehome_dp) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 34 (Valuedual) */
IF ids_apost.SetItem(1, "f34_vatamo_or_valdual", abs(ld_sum_amount_USD)) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 34 (Valuedual) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 35 (Valuedual_dp) */
IF ids_apost.SetItem(1, "f35_vattyp_or_valdual_dp", 2) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 35 (Valuedual_dp) for A-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
IF ids_bpost.SetItem(il_bpost_row, "f03_yr", ids_apost.GetItemNumber(1, "f03_yr")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 03 (Fiscal Year) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 04 (Period) */
IF ids_bpost.SetItem(il_bpost_row, "f04_period", ids_apost.GetItemNumber(1, "f04_period")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 04 (Period) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 12 (Element 2) */
IF ids_bpost.SetItem(il_bpost_row, "f12_el2_b", ids_apost.GetItemString(1, "f12_el2")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 12 (Element 2) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 18 (Element 8) (Always Blank) */
IF ids_bpost.SetItem(il_bpost_row, "f18_el8_b", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 18 (NULL) (Element 8) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_default_bpost")
	Return(-1)
END IF
...
IF ids_bpost.SetItem(il_bpost_row, "f19_custsupp", ids_apost.GetItemString(1, "f19_custsupp")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 19 (CustSupp) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_default_bpost")
	Return(-1)
END IF
...
IF ids_bpost.SetItem(il_bpost_row, "f27_linetype", ids_default_values.GetItemNumber(1, "linetype_analyses_bpost")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 27 (Linetype) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 31 (Valuedoc_dp) */
IF ids_bpost.SetItem(il_bpost_row, "f31_valuedoc_dp", 2) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 31 (Valuedoc_dp) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 33 (Valuehome_dp) */
IF ids_bpost.SetItem(il_bpost_row, "f33_valuehome_dp", 2) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 33 (Valuehome_dp) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 35 (Valuedual_dp) */
IF ids_bpost.SetItem(il_bpost_row, "f35_vattype_or_valdual_dp", 2) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 35 (Valuedual_dp) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_default_bpost")
	Return(-1)
END IF
...
private function integer of_bpost ();/* Transaction for Bareboat Hire bareboat  */
...
if ll_rows < 1 then
	of_messagebox("Retrival error", "Can't retrieve any hire detail. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
	Return(-1)
end if
...
IF ids_bpost.SetItem(il_bpost_row,"f11_el1_b", string(idt_payment_start, "YYYYMM")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 11 (Element 1) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
	Return(-1)
END IF
...
IF ids_bpost.SetItem(il_bpost_row, "f13_el3_b",  ids_coda_elements.getItemString(ii_Brow, "opsa_el3")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 13 (Element 3) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
	Return(-1)
END IF
...
IF	ids_bpost.SetItem(il_bpost_row, "f14_el4_b",ids_coda_elements.getItemString(ii_Brow, "opsa_el4")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 14 (Element 4) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
	Return(-1)
END IF
...
if isNull(ids_coda_elements.getItemString(ii_Brow, "opsa_el5")) then
	IF ids_bpost.SetItem(il_bpost_row, "f15_el5_b", ids_apost.GetItemString(1, "f15_el5")) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 15 (NULL) (Element 5) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_default_bpost")
		Return(-1)
	END IF
else
...
if isNull(ids_coda_elements.getItemString(ii_Brow, "opsa_el6")) then
	IF of_getVoyagenr(idt_payment_start, ls_voyage_nr) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 16 (NULL) (Element 6) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
		Return(-1)
	END IF
	IF ids_bpost.SetItem(il_bpost_row, "f16_el6_b", ids_default_values.GetItemString(1, "prefix_voyage") + ls_voyage_nr) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 16 (NULL) (Element 6) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
...
if isNull(ids_coda_elements.getItemString(ii_Brow, "opsa_el7")) then
	IF ids_bpost.SetItem(il_bpost_row, "f17_el7_b", "") <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 17 (NULL) (Element 7) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
		Return(-1)
	END IF
else
...
if lnv_opsa.of_setrates( istr_trans_input.settle_tc_payment.getItemNumber(1, "contract_id")) = -1 then
	of_messagebox("Retrieve Error", "Retrieval of Bareboat rates failed for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
	destroy  lnv_opsa
	return -1
end if
if istr_trans_input.settle_tc_payment.getItemNumber(1, "monthly_rate") = 1 then
	if lnv_opsa.of_calcAmountMonthlyRate( idt_payment_start , idt_payment_end , ld_hire_amount, 2 ) = -1 then
		of_messagebox("Calculation Error", "Error calculating Bareboat hire for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
		destroy  lnv_opsa
		return -1
	end if
else
	if lnv_opsa.of_calcAmountDailyRate( idt_payment_start , idt_payment_end , ld_hire_amount, 2 ) = -1 then
		of_messagebox("Calculation Error", "Error calculating Bareboat hire for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
		destroy  lnv_opsa
		return -1
	end if
...
IF ld_hire_local > 0 THEN
	IF ids_bpost.SetItem(il_bpost_row, "f29_debitcredit", ids_default_values.GetItemNumber(1, "debitcredit_credit")) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 29 (DebitCredit) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
		Return(-1)
	END IF
ELSE
...
IF ids_bpost.SetItem(il_bpost_row, "f30_valuedoc", abs(ld_hire_local)) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 30 (Valuedoc) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
	Return(-1)
END IF
...
IF ids_bpost.SetItem(il_bpost_row, "f32_valuehome", abs(ld_hire_DKK)) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 32 (Valuehome) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
	Return(-1)
END IF
...
IF ids_bpost.SetItem(il_bpost_row, "f34_vatamo_or_valdual", abs(ld_hire_usd)) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 34 (Valuedual) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
	Return(-1)
END IF

/* Set field no. 41 (Linedescr) */
/* This text must never be changed as it is used to identify the transaction in the log */
IF ids_bpost.SetItem(il_bpost_row, "f41_linedesr", "TC OPSA BB (Hire)") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 41 (Linedescr) for B-post. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
	Return(-1)
END IF

/* Periodiser transaction hvis nødvendigt */
if of_periodiser_bpost(il_bpost_row, idt_payment_start, idt_payment_end) = -1 then
	of_messagebox("Error", "Can't break B-post into periods. Object: u_transaction_opsa_hire_bareboat_coda, function: of_bpost")
	Return(-1)
END IF
...
on u_transaction_opsa_hire_bareboat_coda.create
call super::create
end on

on u_transaction_opsa_hire_bareboat_coda.destroy
call super::destroy
end on
