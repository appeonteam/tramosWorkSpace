$PBExportHeader$u_transaction_opsa_brokercomm_coda.sru
$PBExportComments$TC Hire Out Contract with OPSA setup. Broker Commission is posted as extra transaction
forward
global type u_transaction_opsa_brokercomm_coda from u_transaction_hire
end type
end forward

global type u_transaction_opsa_brokercomm_coda from u_transaction_hire
end type
global u_transaction_opsa_brokercomm_coda u_transaction_opsa_brokercomm_coda
...
IF isNull(id_exrate_tc_to_DKK) OR id_exrate_tc_to_DKK = 0 THEN
	of_messagebox("Error", "Cant get Exchange Rate for TC Currency. Object: u_transaction_opsa_brokercomm_coda, function: of_setExrate()")
	Return -1
END IF
...
if is_tc_currency = "USD" then
	...
	IF isNull(ld_exrate_DKK_to_USD) OR ld_exrate_DKK_to_USD = 0 THEN
		of_messagebox("Error", "Cant get Exchange Rate for USD. Object: Object: u_transaction_opsa_brokercomm_coda, function: of_setExrate()")
		Return -1 
	END IF
...
do while month(date(ldt_calc_start)) <> month(date(f_long2datetime(f_datetime2long(adt_end)-1)))
	...
	ids_bpost.SetItem(il_bpost_row, "f15_el5_b", ids_bpost.getItemString(al_row, "f15_el5_b"))
	...
	IF of_getVoyagenr(ldt_calc_start, ls_voyage_nr) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 16 (NULL) (Element 6) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_periodiser_bpost")
		Return(-1)
	END IF
...
/* get CODA elements for Broker Commission */ 
if ids_coda_elements.retrieve(istr_trans_input.settle_tc_payment.getItemNumber(1, "contract_id"), 3) <> 2 then return -1
...
IF ids_apost.SetItem(1,"trans_type","TCCODARec") <> 1 THEN
	of_messagebox("Set value error", "Cant set transaction type for A-post. Object: u_transaction_opsa_brokercomm_coda, &
									function: of_fill_transaction")
	Return(-1)
END IF

IF ids_apost.SetItem(1,"ch_br_ag_nr", istr_trans_input.agent_no) <> 1 THEN
	of_messagebox("Set value error", "Cant set broker nr for A-post. Object: u_transaction_opsa_brokercomm_coda, &
									function: of_fill_transaction")
	Return(-1)
END IF

IF ids_apost.SetItem(1,"claim_pcn_nr",istr_trans_input.pcn) <> 1 THEN
	of_messagebox("Set value error", "Cant set claim nr for A-post. Object: u_transaction_opsa_brokercomm_coda, &
									function: of_fill_transaction")
	Return(-1)
END IF
////////////////////////////////////////////////////////////////////////////////////////
...
IF ids_apost.SetItem(1, "f06_doccode", ids_default_values.GetItemString(1, "doccode_coda_trans")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 06 (Doccode) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 9 DocDate */
IF ids_apost.SetItem(1, "f09_docdate", istr_trans_input.doc_date) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 09 (Docdate) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
IF ids_apost.SetItem(1, "f13_el3", ids_coda_elements.getItemString(ii_Arow, "opsa_el3") ) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 13 (Own) (Element 3) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 14  Nominal Account  */
IF ids_apost.SetItem(1, "f14_el4",  ids_coda_elements.getItemString(ii_Arow, "opsa_el4") ) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 14 (Foreign) (Element 4) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
ELSE
	of_messagebox("Retrieval error", "Alphanumeric Reference Number for vessel not found. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
if isNull( ids_coda_elements.getItemString(ii_Arow, "opsa_el5")) then
	IF ids_apost.SetItem(1,"f15_el5", ids_default_values.GetItemString(1,"prefix_vessel") + ls_vessel_ref_nr) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 15 (Foreign) (Element 5) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
		Return(-1)
	END IF
else
...
/* Set field no. 16 Voyage  (Element 6) 
	Voyagenumber set after B-post as date is set in of_bpost() */
...
if isNull( ids_coda_elements.getItemString(ii_Arow, "opsa_el7")) then
	IF ids_apost.SetItem(1, "f17_el7", "") <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 17 (Foreign) (Element 7) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
		Return(-1)
	END IF
else
...
IF ids_apost.SetItem(1, "f19_custsupp", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 19 for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 20  (always blank)  */
IF ids_apost.SetItem(1, "f20_invoicenr", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 20 (Invoice No.) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 21 Voucher no. (always blank) */
IF ids_apost.SetItem(1, "f21_vouchernr", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 21 (Voucher No.) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 22 Control no. (always blank) */
IF ids_apost.SetItem(1, "f22_controlnr", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 22 (Control No.) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 23  APM Supplier - same as nominal account (always blank)   */
IF ids_apost.SetItem(1, "f23_paytype_or_sup", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 23 (APM Supplier) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 28  Currency Code  */
IF ids_apost.SetItem(1, "f28_curdoc", is_tc_currency) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 28 (CurDoc) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF

/* Set field no. 41  Linedescr  */
/* This text must never be changed as it is used to identify the transaction in the log */
IF ids_apost.SetItem(1, "f41_linedesr", "TC OPSA (BrokerComm)") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 41 (Linedescr) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
IF ids_apost.SetItem(1, "payment_id", istr_trans_input.settle_tc_payment.getItemNumber(1, "payment_id")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field Payment_ID for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
	Return(-1)
END IF
...
/* Special setup for commission 1 B-post = 1 A post. Therefore dublicate A-posts if several B-posts */
ll_rows = ids_bpost.rowCount()
/* Create as many A-posts as B-posts */
for ll_row = 1 to (ll_rows -1)     
	ids_apost.RowsCopy(1, 1, Primary!, ids_apost, 999, Primary!)
next	


for ll_row = 1 to ll_rows
	/* Set field no. 11  Activity Periode  */
	IF ids_apost.SetItem(ll_row, "f11_el1", ids_bpost.getItemString(ll_row, "f11_el1_b")) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 11 (Element 1) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
		Return(-1)
	END IF

	/* Set field no. 16 Voyage  (Element 6) */
	if isNull(ids_coda_elements.getItemString(ii_Arow, "opsa_el6")) then
		IF ids_apost.SetItem(ll_row, "f16_el6", ids_bpost.getItemString(ll_row, "f16_el6_b")) <> 1 THEN
			of_messagebox("Set value error", "Cant set Field 16 (Element 6) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
			Return(-1)
		END IF
	else
		IF ids_apost.SetItem(ll_row, "f16_el6", ids_coda_elements.getItemString(ii_Arow, "opsa_el6")) <> 1 THEN
			of_messagebox("Set value error", "Cant set Field 16 (NULL) (Element 6) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
			Return(-1)
		END IF
	end if	

	/* Set field no. 08 DocLineNumber for all B-posts . As there here is one to one relation always 2*/
	IF ids_bpost.SetItem(ll_row, "f08_doclinenum_b", 2) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 08 (Doclinenumber) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_bpost")
		Return(-1)
	end if

	/* Set field no. 29 (DebitCredit) */
	if ids_bpost.getItemNumber(ll_row, "f29_debitcredit") = ids_default_values.GetItemNumber(1, "debitcredit_debit") then
		IF ids_apost.SetItem(ll_row, "f29_debitcredit", ids_default_values.GetItemNumber(1, "debitcredit_credit")) <> 1 THEN
			of_messagebox("Set value error", "Cant set Field 29 (DebitCredit) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
			Return(-1)
		END IF
	else
		IF ids_apost.SetItem(ll_row, "f29_debitcredit", ids_default_values.GetItemNumber(1, "debitcredit_debit")) <> 1 THEN
			of_messagebox("Set value error", "Cant set Field 29 (DebitCredit) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
			Return(-1)
		END IF
	end if
	...
	IF ids_apost.SetItem(ll_row, "f30_valuedoc", ids_bpost.getItemNumber(ll_row, "f30_valuedoc")) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 30 (Valuedoc) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
		Return(-1)
	END IF
	
	/* Set field no. 31 (Valuedoc_dp) */
	IF ids_apost.SetItem(ll_row, "f31_valuedoc_dp", 2) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 31 (Valuedoc_dp) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
		Return(-1)
	END IF
	
	/* Set field no. 32 (Valuehome) */
	IF ids_apost.SetItem(ll_row, "f32_valuehome", ids_bpost.getItemNumber(ll_row, "f32_valuehome")) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 32 (Valuehome) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
		Return(-1)
	END IF
	
	/* Set field no. 33 (Valuehome_dp) */
	IF ids_apost.SetItem(ll_row, "f33_valuehome_dp", 2) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 33 (Valuehome_dp) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
		Return(-1)
	END IF
	
	/* Set field no. 34 (Valuedual) */
	IF ids_apost.SetItem(ll_row, "f34_vatamo_or_valdual", ids_bpost.getItemNumber(ll_row, "f34_vatamo_or_valdual")) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 34 (Valuedual) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
		Return(-1)
	END IF
	
	/* Set field no. 35 (Valuedual_dp) */
	IF ids_apost.SetItem(ll_row, "f35_vattyp_or_valdual_dp", 2) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 35 (Valuedual_dp) for A-post. Object: u_transaction_opsa_brokercomm_coda, function: of_fill_transaction")
		Return(-1)
	END IF
next	
...
IF ids_bpost.SetItem(il_bpost_row, "f03_yr", ids_apost.GetItemNumber(1, "f03_yr")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 03 (Fiscal Year) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 04 (Period) */
IF ids_bpost.SetItem(il_bpost_row, "f04_period", ids_apost.GetItemNumber(1, "f04_period")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 04 (Period) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 12 (Element 2) */
IF ids_bpost.SetItem(il_bpost_row, "f12_el2_b", ids_apost.GetItemString(1, "f12_el2")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 12 (Element 2) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 18 (Element 8) (Always Blank) */
IF ids_bpost.SetItem(il_bpost_row, "f18_el8_b", "") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 18 (NULL) (Element 8) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_default_bpost")
	Return(-1)
END IF
...
IF ids_bpost.SetItem(il_bpost_row, "f19_custsupp", ids_apost.GetItemString(1, "f19_custsupp")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 19 (CustSupp) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_default_bpost")
	Return(-1)
END IF
...
IF ids_bpost.SetItem(il_bpost_row, "f27_linetype", ids_default_values.GetItemNumber(1, "linetype_analyses_bpost")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 27 (Linetype) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 31 (Valuedoc_dp) */
IF ids_bpost.SetItem(il_bpost_row, "f31_valuedoc_dp", 2) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 31 (Valuedoc_dp) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 33 (Valuehome_dp) */
IF ids_bpost.SetItem(il_bpost_row, "f33_valuehome_dp", 2) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 33 (Valuehome_dp) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_default_bpost")
	Return(-1)
END IF

/* Set field no. 35 (Valuedual_dp) */
IF ids_bpost.SetItem(il_bpost_row, "f35_vattype_or_valdual_dp", 2) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 35 (Valuedual_dp) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_default_bpost")
	Return(-1)
END IF
...
private function integer of_bpost ();/* Transaction for OPSA Broker Commission */

n_ds			lds_hire_detail
long			ll_rows
decimal{0}	ld_brokercomm_local, ld_brokercomm_DKK, ld_brokercomm_USD
string			ls_voyage_nr
...
if ll_rows < 1 then
	of_messagebox("Retrival error", "Can't retrieve any hire detail. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
	Return(-1)
end if
...
IF ids_bpost.SetItem(il_bpost_row,"f11_el1_b", string(idt_payment_start, "YYYYMM")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 11 (Element 1) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
	Return(-1)
END IF
...
IF ids_bpost.SetItem(il_bpost_row, "f13_el3_b",  ids_coda_elements.getItemString(ii_Brow, "opsa_el3")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 13 (Element 3) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
	Return(-1)
END IF
...
IF	ids_bpost.SetItem(il_bpost_row, "f14_el4_b",ids_coda_elements.getItemString(ii_Brow, "opsa_el4")) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 14 (Element 4) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
	Return(-1)
END IF
...
if isNull(ids_coda_elements.getItemString(ii_Brow, "opsa_el5")) then
	IF ids_bpost.SetItem(il_bpost_row, "f15_el5_b", ids_apost.GetItemString(1, "f15_el5")) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 15 (NULL) (Element 5) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_default_bpost")
		Return(-1)
	END IF
else
...
if isNull(ids_coda_elements.getItemString(ii_Brow, "opsa_el6")) then
	IF of_getVoyagenr(idt_payment_start, ls_voyage_nr) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 16 (NULL) (Element 6) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
		Return(-1)
	END IF
	IF ids_bpost.SetItem(il_bpost_row, "f16_el6_b", ids_default_values.GetItemString(1, "prefix_voyage") + ls_voyage_nr) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 16 (NULL) (Element 6) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
...
if isNull(ids_coda_elements.getItemString(ii_Brow, "opsa_el7")) then
	IF ids_bpost.SetItem(il_bpost_row, "f17_el7_b", "") <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 17 (NULL) (Element 7) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
		Return(-1)
	END IF
else
...
/* Set field no. 29 (DebitCredit) */
ld_brokercomm_local = istr_trans_input.opsa_broker_comm
IF ld_brokercomm_local > 0 THEN
	IF ids_bpost.SetItem(il_bpost_row, "f29_debitcredit", ids_default_values.GetItemNumber(1, "debitcredit_credit")) <> 1 THEN
		of_messagebox("Set value error", "Cant set Field 29 (DebitCredit) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
		Return(-1)
	END IF
ELSE
...
/* Set field no. 30 (Valuedoc) */
IF ids_bpost.SetItem(il_bpost_row, "f30_valuedoc", abs(ld_brokercomm_local)) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 30 (Valuedoc) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
	Return(-1)
END IF

/* Set field no. 32 (Valuehome) */
ld_brokercomm_DKK = ld_brokercomm_local * id_exrate_TC_to_DKK / 100
IF ids_bpost.SetItem(il_bpost_row, "f32_valuehome", abs(ld_brokercomm_DKK)) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 32 (Valuehome) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
	Return(-1)
END IF

/* Set field no. 34 (Valuedual) */
ld_brokercomm_USD = ld_brokercomm_local * id_exrate_TC_to_USD / 100
IF ids_bpost.SetItem(il_bpost_row, "f34_vatamo_or_valdual", abs(ld_brokercomm_usd)) <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 34 (Valuedual) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
	Return(-1)
END IF

/* Set field no. 41 (Linedescr) */
/* This text must never be changed as it is used to identify the transaction in the log */
IF ids_bpost.SetItem(il_bpost_row, "f41_linedesr", "TC OPSA (BrokerComm)") <> 1 THEN
	of_messagebox("Set value error", "Cant set Field 41 (Linedescr) for B-post. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
	Return(-1)
END IF

/* Periodiser transaction hvis nødvendigt */
if of_periodiser_bpost(il_bpost_row, idt_payment_start, idt_payment_end) = -1 then
	of_messagebox("Error", "Can't break B-post into periods. Object: u_transaction_opsa_brokercomm_coda, function: of_bpost")
	Return(-1)
END IF
...
long ll_transkey, ll_Arows, ll_Brows, ll_rowno

//f_datastore_spy(ids_apost)
//f_datastore_spy(ids_bpost)

ll_Arows = ids_apost.Rowcount()
ll_Brows = ids_bpost.Rowcount( )
if ll_Arows <> ll_Brows then
	of_messagebox("Generate transaction error", "There is NOT an equal number of Apost and Bpost. Object: u_transaction_opsa_brokercomm_coda, function: of_save")
	ROLLBACK;
	Return(-1)
end if

IF ids_apost.RowCount() > 0 THEN
	IF ids_apost.Update() <> 1 THEN
		of_messagebox("Error","Update of ids_Apost went wrong" + SQLCA.SqlErrText+ "  Object: u_transaction_opsa_brokercomm_coda, function: of_save")
		ROLLBACK;
		Return(-1)
	end if	
	
	/* fill all B posts with corresponding transaction key */
	for ll_rowno = 1 to ll_Arows	
		ll_transkey = ids_apost.GetItemNumber(ll_rowno,"trans_key")
		IF IsNull(ll_transkey) OR ll_transkey = 0 THEN
			of_messagebox("Generate transaction error", "No value found for transaction key.  Object: u_transaction_opsa_brokercomm_coda, function: of_save")
			ROLLBACK;
			Return(-1)
		ELSE
			ids_bpost.SetItem(ll_rowno, "trans_key", ids_apost.getItemNumber(ll_rowno, "trans_key"))
		end if
	next
	...
	ELSE
		ROLLBACK;
		of_messagebox("Error","ids_Apost ok, but ids_Bpost went wrong in update.  Object: u_transaction_opsa_brokercomm_coda, function: of_save")
		Return(-1)
	END IF
ELSE
	of_messagebox("Generate transaction","No CODA transaction Generated. Probably because all CODA transactions already posted!  Object: u_transaction_opsa_brokercomm_coda, function: of_save")
	Return(1)
END IF

end function

on u_transaction_opsa_brokercomm_coda.create
call super::create
end on

on u_transaction_opsa_brokercomm_coda.destroy
call super::destroy
end on
