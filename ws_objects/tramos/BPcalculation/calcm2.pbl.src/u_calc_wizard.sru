$PBExportHeader$u_calc_wizard.sru
$PBExportComments$Wizard subobject - used by u_calculation
forward
global type u_calc_wizard from u_calc_base_sqlca
end type
type dw_wizard from uo_datawindow within u_calc_wizard
end type
type dw_calc_via_dddw from u_datawindow_sqlca within u_calc_wizard
end type
type dw_ballasted_speed from u_datawindow_sqlca within u_calc_wizard
end type
type dw_laden_speed from u_datawindow_sqlca within u_calc_wizard
end type
type gb_1 from groupbox within u_calc_wizard
end type
type s_fixup from structure within u_calc_wizard
end type
type s_cargodata from structure within u_calc_wizard
end type
end forward
...
global u_calc_wizard u_calc_wizard
...
Else
	...
	iuo_calc_nvo.iuo_calc_itinerary.uf_clear_cache(true)
	ib_active = false
	Return(1)
End if
end function
