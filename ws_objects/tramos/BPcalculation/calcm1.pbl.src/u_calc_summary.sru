$PBExportHeader$u_calc_summary.sru
$PBExportComments$summary subobject - used by u_calculation
forward
global type u_calc_summary from u_calc_base_sqlca
end type
type cb_refresh_bunker from commandbutton within u_calc_summary
end type
type dw_calc_summary from u_datawindow_sqlca within u_calc_summary
end type
type dw_calc_ballast from u_datawindow_sqlca within u_calc_summary
end type
type dw_calc_summary_list from u_datawindow_sqlca within u_calc_summary
end type
end forward
...
global u_calc_summary u_calc_summary
...
If dw_calc_ballast.GetItemString(1,"port_code")<>dw_calc_summary.GetItemString(1,"cal_calc_ballast_from") Then
...
If dw_calc_ballast.GetItemString(2,"port_code")<>dw_calc_summary.GetItemString(1,"cal_calc_ballast_to") Then
		MessageBox("System error", "Ballast codes are different~r~n~r~n "+ &
...
If (li_status >= 4) Then 
	...
	dw_calc_summary.SetColumn("locked")
	dw_calc_summary.vscrollbar = true
Else 
	li_locked = 0
	dw_calc_summary.vscrollbar = false
End if
...
dw_calc_summary.SetItem(1,"cal_vest_type_id",al_vessel_type_id)
dw_calc_summary.SetItem(1,"cal_calc_vessel_id",al_vessel_id)
dw_calc_summary.SetItem(1,"cal_clrk_id",al_clarkson_id)

iuo_calc_nvo.uf_get_vessel_data(al_vessel_type_id, al_vessel_id, al_clarkson_id)

dw_calc_summary.SetItem(1,"cal_calc_fo_price", iuo_calc_nvo.istr_calc_vessel_data.d_fo_price)
dw_calc_summary.SetItem(1,"cal_calc_do_price", iuo_calc_nvo.istr_calc_vessel_data.d_do_price)
dw_calc_summary.SetItem(1,"cal_calc_mgo_price", iuo_calc_nvo.istr_calc_vessel_data.d_mgo_price)
...
dw_calc_summary.Modify("DataWindow.ReadOnly=Yes")
dw_calc_summary_list.Modify("DataWindow.ReadOnly=Yes")
end event
...
this.Control[iCurrent+1]=this.cb_refresh_bunker
this.Control[iCurrent+2]=this.dw_calc_summary
this.Control[iCurrent+3]=this.dw_calc_ballast
this.Control[iCurrent+4]=this.dw_calc_summary_list
end on

on u_calc_summary.destroy
call super::destroy
...
string text = "Refresh Bunker Prices"
end type
...
integer height = 288
integer taborder = 30
string dataobject = "d_calc_ballast"
end type
...
If il_estimated_calc_id > 0 Then
	uo_sql_util = CREATE u_sql_util
/* Below modified when going to PB10 from PB07 */
//	ls_sql = LeftTrim(This.GetSQLPreview())
	ls_sql = LeftTrim(SQLsyntax)
	...
	If Left(ls_sql,6)="UPDATE" Then
		...
		uo_sql_util.uf_remove_where(ls_sql, "CAL_BALL_ID")
