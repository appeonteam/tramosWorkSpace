$PBExportHeader$w_calc_recalc.srw
$PBExportComments$Window for recalcing all calculations
forward
global type w_calc_recalc from mt_w_sheet_calc
end type
type uo_calculation from u_atobviac_calculation within w_calc_recalc
end type
type cb_cancel from commandbutton within w_calc_recalc
end type
type dw_status_log from uo_datawindow within w_calc_recalc
end type
type cb_recalc from commandbutton within w_calc_recalc
end type
type st_status from statictext within w_calc_recalc
end type
type st_no_calc from statictext within w_calc_recalc
end type
type st_no_calculations from statictext within w_calc_recalc
end type
type st_elapsed_time from statictext within w_calc_recalc
end type
type st_time from statictext within w_calc_recalc
end type
type st_avgtime from statictext within w_calc_recalc
end type
type st_avg_time from statictext within w_calc_recalc
end type
type st_timeleft_ from statictext within w_calc_recalc
end type
type st_timeleft from statictext within w_calc_recalc
end type
type cbx_no_reload from uo_cbx_base within w_calc_recalc
end type
type rb_all from uo_rb_base within w_calc_recalc
end type
type rb_failed from uo_rb_base within w_calc_recalc
end type
type rb_ok from uo_rb_base within w_calc_recalc
end type
type cbx_save from uo_cbx_base within w_calc_recalc
end type
type rb_autocomp_none from uo_rb_base within w_calc_recalc
end type
type rb_autocomp_read from uo_rb_base within w_calc_recalc
end type
type rb_autocomp_write from uo_rb_base within w_calc_recalc
end type
type sle_autocomp_filename from singlelineedit within w_calc_recalc
end type
type st_1 from statictext within w_calc_recalc
end type
type cbx_adhook_print from uo_cbx_base within w_calc_recalc
end type
type gb_status from groupbox within w_calc_recalc
end type
type cb_print from commandbutton within w_calc_recalc
end type
type gb_time from uo_gb_base within w_calc_recalc
end type
type gb_show from uo_gb_base within w_calc_recalc
end type
type cb_close from commandbutton within w_calc_recalc
end type
type gb_options from uo_gb_base within w_calc_recalc
end type
type gb_autocompare from uo_gb_base within w_calc_recalc
end type
type dw_status_log_check from datawindow within w_calc_recalc
end type
type cb_1 from uo_cb_base within w_calc_recalc
end type
end forward
...
global w_calc_recalc w_calc_recalc

type variables
Long il_id_list[], il_id_count
...
// Define the ID cursor
DECLARE idcursor CURSOR FOR
SELECT TOP 1000 CAL_CALC_ID
FROM CAL_CALC
WHERE USE_ATOBVIAC_DISTANCE = 1
AND CAL_CALC_STATUS > 4
AND CAL_CALC_STATUS < 7
order by CAL_CALC_ID DESC;

OPEN idcursor;

il_id_count = 1

// and fetch it into the list
DO WHILE SQLCA.SQLCode = 0
	FETCH  idcursor 
	INTO :il_id_list[il_id_count];
...
public subroutine documentation ();/********************************************************************
   ObjectName: w_calc_recalc
	</OBJECT>
		Recalculate a calculation
	<USAGE> </USAGE>
   <ALSO>
		otherobjs
	</ALSO>
   <HISTORY>
   	...
   	07-26-2013 CR3183       WWA048        When the data in calculated status calculation is changed,
...
this.Control[iCurrent+3]=this.dw_status_log
this.Control[iCurrent+4]=this.cb_recalc
this.Control[iCurrent+5]=this.st_status
this.Control[iCurrent+6]=this.st_no_calc
this.Control[iCurrent+7]=this.st_no_calculations
this.Control[iCurrent+8]=this.st_elapsed_time
this.Control[iCurrent+9]=this.st_time
this.Control[iCurrent+10]=this.st_avgtime
this.Control[iCurrent+11]=this.st_avg_time
this.Control[iCurrent+12]=this.st_timeleft_
this.Control[iCurrent+13]=this.st_timeleft
this.Control[iCurrent+14]=this.cbx_no_reload
this.Control[iCurrent+15]=this.rb_all
this.Control[iCurrent+16]=this.rb_failed
this.Control[iCurrent+17]=this.rb_ok
this.Control[iCurrent+18]=this.cbx_save
this.Control[iCurrent+19]=this.rb_autocomp_none
this.Control[iCurrent+20]=this.rb_autocomp_read
this.Control[iCurrent+21]=this.rb_autocomp_write
this.Control[iCurrent+22]=this.sle_autocomp_filename
this.Control[iCurrent+23]=this.st_1
this.Control[iCurrent+24]=this.cbx_adhook_print
this.Control[iCurrent+25]=this.gb_status
this.Control[iCurrent+26]=this.cb_print
this.Control[iCurrent+27]=this.gb_time
this.Control[iCurrent+28]=this.gb_show
this.Control[iCurrent+29]=this.cb_close
this.Control[iCurrent+30]=this.gb_options
this.Control[iCurrent+31]=this.gb_autocompare
this.Control[iCurrent+32]=this.dw_status_log_check
this.Control[iCurrent+33]=this.cb_1
end on

on w_calc_recalc.destroy
call super::destroy
...
//							'"'+dw_status_log_check.GetItemString(ll_row_check, "status"))
	...
	st_avg_time.text = wf_time_to_Str(ll_tmp / (ll_count - 1))
	...
	st_timeleft.text = wf_time_to_str((ll_tmp / (ll_count -1)) * (il_id_count - ll_count -1 ))	
