$PBExportHeader$w_calc_consumption.srw
$PBExportComments$Consumption window for the vessel window, can accept new data
forward
global type w_calc_consumption from mt_w_response_calc
end type
type dw_calc_consumption_data from u_datawindow_sqlca within w_calc_consumption
end type
type dw_calc_consumption_list from u_datawindow_sqlca within w_calc_consumption
end type
type cb_delete from uo_cb_base within w_calc_consumption
end type
type cb_new from uo_cb_base within w_calc_consumption
end type
type cb_save from uo_cb_base within w_calc_consumption
end type
type st_1 from uo_st_base within w_calc_consumption
end type
type gb_1 from uo_gb_base within w_calc_consumption
end type
type gb_2 from uo_gb_base within w_calc_consumption
end type
type cb_close from uo_cb_base within w_calc_consumption
end type
type st_2 from uo_st_base within w_calc_consumption
end type
end forward
...
global w_calc_consumption w_calc_consumption
...
DO
	...
	If ll_rownum > 0 Then
		...
		If (ll_type = 1 Or ll_type = 2) And (ll_speed = 0) Then
			MessageBox("Notice", "speed must be above zero for loaded and ballasted")
			lb_continue = False
			lb_return = False
		End If
	End if
...
setNull( istr_calc_vessel_id.l_vessel_id)
dw_calc_consumption_list.Retrieve(istr_calc_vessel_id.l_vessel_type_id, istr_calc_vessel_id.l_vessel_id, istr_calc_vessel_id.l_clarkson_id)
COMMIT;
...
dw_calc_consumption_data.Enabled = True
dw_calc_consumption_list.SetFocus()
...
type st_hidemenubar from mt_w_response_calc`st_hidemenubar within w_calc_consumption
end type
...
string text = "To make a new entry into the consumption table: Push the new button."
