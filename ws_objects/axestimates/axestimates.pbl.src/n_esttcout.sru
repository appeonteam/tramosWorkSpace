$PBExportHeader$n_esttcout.sru
$PBExportComments$TC-OUT estimatations
forward
global type n_esttcout from n_estimate
end type
end forward

global type n_esttcout from n_estimate
end type
global n_esttcout n_esttcout
...
if istr_app.b_client = false then
	...
	_addmessage( this.classdefinition, "of_writeoutdebug()", "voyage first arr dt :" + string(istr_tcout.dt_firstarrival, "dd/mm/yyyy hh:mm"), "")		
...
if ls_voyagenr <> "" then
	...
	SELECT min(PORT_ARR_DT) INTO :ldt_firstarrival
	  FROM ( SELECT min(PORT_ARR_DT) PORT_ARR_DT 
		        FROM POC
		      ...
		      UNION ALL
			   SELECT min(PORT_ARR_DT) PORT_ARR_DT
		        FROM POC_EST
		       WHERE VESSEL_NR = :li_vesselnr
			      AND LEN(LTRIM(RTRIM(VOYAGE_NR))) > 5
			      AND LEFT(LTRIM(VOYAGE_NR), 5) = :ls_voyagenr
			      AND PURPOSE_CODE = "DEL") TMP_POC;
		
	if isnull(ldt_firstarrival) then
		SELECT min(PORT_ARR_DT) INTO :ldt_firstarrival
		  FROM ( SELECT min(PORT_ARR_DT) PORT_ARR_DT
		           FROM POC
		          WHERE VESSEL_NR = :li_vesselnr
		         ...
		         SELECT min(PORT_ARR_DT) PORT_ARR_DT
		          ...
		          WHERE VESSEL_NR = :li_vesselnr
			         AND LEN(LTRIM(RTRIM(VOYAGE_NR))) > 5
			         AND LEFT(LTRIM(VOYAGE_NR), 5) = :ls_voyagenr) TMP_POC;
	end if		
end if
...
on n_esttcout.create
call super::create
end on

on n_esttcout.destroy
call super::destroy
end on
