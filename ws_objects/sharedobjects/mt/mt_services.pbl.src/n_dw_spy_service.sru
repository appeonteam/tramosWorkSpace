$PBExportHeader$n_dw_spy_service.sru
$PBExportComments$Used to monitor datawindows and datastores
forward
global type n_dw_spy_service from mt_n_baseservice
end type
end forward

global type n_dw_spy_service from mt_n_baseservice
end type
global n_dw_spy_service n_dw_spy_service
...
public subroutine of_capturesql (boolean ab_enabled)
end prototypes

public subroutine documentation ();/********************************************************************
   ObjectName: n_dw_spy_service
  ...
  29/06/16	CR4276	AGL027		Minor change to enable data spy option for server app
********************************************************************/
...
ids_monitormessage.setItem(ll_row, "timestamp", datetime(today(), Now()))
...
public subroutine of_capturesql (boolean ab_enabled);choose case ab_enabled
	case TRUE
		if NOT isValid(w_sqlsyntax_spy) then
			openwithparm(w_sqlsyntax_spy, ids_monitormessage) 
		end if
	case FALSE 
		if isValid(w_sqlsyntax_spy) then
			close ( w_sqlsyntax_spy )
		end if
end choose

end subroutine

on n_dw_spy_service.create
call super::create
end on

on n_dw_spy_service.destroy
call super::destroy
end on
...
ids_monitorMessage.dataObject = "d_ex_tb_sqlsyntax"
if isvalid(w_sqlsyntax_spy) then
	w_sqlsyntax_spy.dw_sql.sharedata(ids_monitorMessage)
end if
