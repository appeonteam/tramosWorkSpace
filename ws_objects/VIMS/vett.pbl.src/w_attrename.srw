...
type cb_3 from commandbutton within w_attrename
end type
...
li_Pos = Len(g_obj.ObjString) 

Do
	li_Pos --
Loop Until Mid(g_obj.ObjString, li_Pos, 1) = "." or li_Pos <= 1

li_Pos --

If li_Pos = 0 then li_Pos = Len(g_obj.ObjString) 

sle_name.Text = g_obj.ObjString
...
long backcolor = 67108864
string text = "Renaming the extension may cause the file to become unusable. Do not change the extension unless necessary."
...
g_obj.ObjString = Trim(sle_name.text)

If Len(g_obj.ObjString)<5 then 
	Messagebox("Invalid Name", "The full file name must be at least 5 characters including the extension.",Exclamation!)
	Return
End If

For li_Pos = 1 to Len(g_obj.ObjString)
	If Pos("ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&(){}[];',.-=+_ ", Upper(Mid(g_obj.ObjString,li_Pos,1))) = 0 then
		Messagebox("Invalid Name", "The file name contains one or more invalid characters.",Exclamation!)
		Return		
	End If
Next

If (Left(g_obj.Objstring,1) = ".") or (Right(g_obj.Objstring,1) = ".") then
