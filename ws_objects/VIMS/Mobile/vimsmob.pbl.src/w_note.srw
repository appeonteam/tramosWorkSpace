...
f_Write2Log("w_Note Open")

mle_note.backcolor = 15790320

Select NOTETYPE, NOTE_TEXT into :li_Type, :ls_Note from VETT_NOTES where NOTES_ID = :g_Obj.ParamLong;

If SQLCA.Sqlcode<>0 then 
	MessageBox("DB Error", "Could not retrive text from Database.~n~n" + Sqlca.Sqlerrtext,Exclamation!)
	f_Write2Log("w_Note > Retrieve Failed: " + SQLCA.SQLErrText)
	Rollback;
Else
	Commit;
	Choose Case li_Type
