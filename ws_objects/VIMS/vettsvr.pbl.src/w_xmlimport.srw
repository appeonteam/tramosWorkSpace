$PBExportHeader$w_xmlimport.srw
forward
...
FileDelete(gs_importPath + as_FileName)
...
For li_Count = 1 to li_Total  // loop thru files
	...
	ElseIf Right(lb_Files.Text(li_Count),5)=".vpkg" Then		  // otherwise handle inspection
...
ElseIf Upper(as_Action) = "ACCEPTED" then  // If confirmation mail
	Update VETT_ITEM Set CAP_GEN = (Case When CAP_GEN = 0 Then 1 Else CAP_GEN End) Where ITEM_ID = :al_ItemID;
